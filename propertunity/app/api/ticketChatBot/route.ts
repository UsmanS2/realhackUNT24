...
function cleanAndParseJsonString(input: string): TicketData {
  ...
  try {
    ...
    jsonObject = { title: "", description: "", priority: 1, date: "" }; // Fallback object
  }
    ...
    if (Stage === "") {
      ...
      if (message.toLowerCase().includes("y")) {
        ...
        await closedCollection.insertOne({
          title: (data[data.length - 1] as TicketData).title,
          message: (data[data.length - 1] as TicketData).description,
          assignment: "John",
          danger_level: (data[data.length - 1] as TicketData).priority,
          date: getCurrentDateFormatted(),
          issue_date: (data[data.length - 1] as TicketData).date,
          progress: 0,
          image: "",
          cost: 0,
          opp_cost: 0,
